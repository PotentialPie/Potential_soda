...
class BaseRotatingHandler(SafeFileHandler):
    ...
    def __init__(self, filename, mode, encoding=None, delay=0):
        ...
        if codecs is None:
            encoding = None
        SafeFileHandler.__init__(self, filename, mode, encoding, delay)
        self.mode = mode
        self.encoding = encoding
    ...
    def emit(self, record):
        ...
        try:
            if self.shouldRollover(record):
                self.doRollover()
            SafeFileHandler.emit(self, record)
        except (KeyboardInterrupt, SystemExit):
            raise
        except:
            self.handleError(record)
...
class TestTimedRotatingFileHandler(BaseRotatingHandler):
    """
    ...
    def doRollover(self):
        ...
        # if str(time.strftime(self.suffix, timeTuple)) == str(self.baseFilename).split('logger.log.')[1]:
...
def init_log():
    ...
    log_dir_name = "logs/segmentation_log"
    # 日志名称
    # log_file_name = 'logger.log' + '.' + time.strftime('%Y-%m-%d', time.localtime(time.time()))
    # log_file_name = 'logger-' + time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())) + '.log'
    ...
    file_log_handler = TestTimedRotatingFileHandler(log_file_str, when='MIDNIGHT', interval=1, backupCount=2)
    # file_log_handler = TestTimedRotatingFileHandler(log_file_str, when='s', interval=2, backupCount=2)
