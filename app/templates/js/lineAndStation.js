...
//绘制地铁路线
function setLinePath(options, r) {
	var g = options["g"],
	...
	for(var i = 1; i < g.length + 1; i++) {
		var pathLine = svgCreate('g', {
			id: 'map' + num + 'Line-' + lineName[i - 1]
		});
		$("#linePath" + num).append(pathLine);
		var linearLine = svgCreate('g', {
			id: 'map' + num + 'linearLine-' + i
		});
		$("#linearColor" + num).append(linearLine);
		for(var j = 0; j < pathData[i - 1].length; j++) {
			//设置渐变色
			var linearPath = svgCreate('g', {
				id: 'map' + num + 'linearPath' + i + '-' + j
			});
			...
			var path = svgCreate('path', {
				...
				'stroke-width': 2,
				'fill': 'none',
				'id': 'map' + num + lineName[i - 1] + j,
				'name': stationTF[i - 1][j]
			});
...
//绘制地铁站点
function getStationPassenger(options, r) {
	var g = options["g"],
		sit = options["sit"],
	...
	for(var i = 0; i < g.length; i++) {
		var fillColor;
		var siteNode = svgCreate('g', {
			'id': 'map' + num + 'Sit-' + lineName[i]
		});
...
function stationDom(options) {
	var id = options["mapId"],
		...
		siteName = options["siteName"],
		lineName = options["lineName"];
	var sitDom = svgCreate('g', {
		id: 'siteDomPoint' + num
	});
	$(id).append(sitDom);
	for(var i = 0; i < g.length; i++) {
		var color = 'url(#red_none2)';
		var sitDomNode = svgCreate('g', {
			'id': 'map' + num + 'SitDom-' + lineName[i]
		});
		...
		for(var j = 0; j < sit[i].length; j++) {
			...
			var cy = sit[i][j].split(',')[1];
			var path = 'M' + cx + ',' + cy;
			var circleDom = svgCreate('circle', {
				'id': 'map' + num + 'circleDom-' + i + 'c' + j,
				'cx': 0,
				'cy': 0,
				'r': 3,
				'fill': 'transparent',
			...
			var domPath = svgCreate('animateMotion', {
				'path': path
			});
			...
			$(circleDom).on('click', function() {
				...
				var domScale = svgCreate('animateTransform', {
